...
// Add post_id to he query string so that we can use for delete function                                                             
	$query = "SELECT post_id, user_name,post_date,subject,message FROM forum WHERE user_name = ? ";
	...
	if (mysqli_num_rows($result) > 0) {
		...
		{
			//create variable $post_id as parameter, and use it for delete function 
			$post_id = htmlspecialchars($row['post_id'], ENT_QUOTES);
			...
			echo '<tr>
				...
				<td scope="row">' . $message . '</td>
		                <td><a href="delete_post.php?id=' . $post_id . '">Delete</a></td>
				</tr>';  
		}
		echo '</table>' ;
	}
